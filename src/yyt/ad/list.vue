    ...
    <section class="plr-sm">
        ...
        <my-table :data="adList" :config="tableConfig">
            <div slot="operating" slot-scope="ad">
                ...
                <el-button @click="deleteADItem(ad.item,ad.index)" icon="el-icon-delete" circle
...
//        this.showEditModal = true;
                if (this.adItem.id) {
                    that.$router.push('/ad/edit/?adId=' + this.adItem.id);
                } else {
                    that.$router.push('/ad/edit/');
                }
            ...
            getADList() {
                const that = this;
                AD.prototype.getList(that.type).then(res => {
